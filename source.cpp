...
int login()
{
  ...
  do
  {
    ...
    cout << "Password : ";
    cin >> password;
    if(strcmp(username,"aniket123")==0 && strcmp(password,"AniketP02@")==0)
      {
        cout << "\n\n";
        cout << "\t\t\t\t";
...
private:
  ...
  descp des;
  public:
  int ncheck(char arr[5]);                 // function to check that input is non-numeric
...
int criminal::ncheck (char arr[5])
{
  int i=0;
  while (arr[i]!='\0')
  {
...
{
  if(( (a[0]=='m')||(a[0]=='M')||(a[5]=='F')||(a[0]=='f'))&&(a[1]=='\0') )
  {
    return 1;
  }
  else
...

{
  ...
  fflush(stdin);
  fstream filin;
  filin.open("Jail_Information.txt",ios::app);
...
void criminal::delete_rec(char con_no[]) // function for deleting a record
{
  fstream filin;
  fflush(stdin);
  filin.open("Jail_Information.txt",ios::in|ios::ate);
  int q = filin.tellg();
  ...
  filin.close();
  temp.close();
  filin.open("Jail_Information.txt",ios::out);
...
int criminal::dcheck(char n[]) // function to check correct format of date
{
  system("cls");
  fstream file;
  file.open("Jail_Information.txt",ios::in|ios::ate|ios::binary);

  int x = 0, count = 0, c;
  int q = file.tellg();
  c = q/sizeof(criminal);
  file.close();
  file.open("Jail_Information.txt",ios::in|ios::binary);
  for(int i=0;i<c;i++)
  {
...
void criminal::modify() // function to modify record
{
  ...
  if(check != 0)
  {
    file.open("Jail_Information.txt",ios::out|ios::ate);
    int d = 0;
    cout << "\n\n\t\tRECORD FOUND !\n\n";
...
int criminal::concheck(char b[])
{
  ...
  int check=0;
  file.open("Jail.data",ios::in|ios::ate);
  int q=file.tellg();
